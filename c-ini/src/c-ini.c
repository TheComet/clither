...
static int cstr_equal(const char* s1, struct str_view s2, const char* data)
{
...
static int print_parser_error(struct parser* p, const char* fmt, ...)
{
    va_list         ap;
    struct str_view loc;
...
enum token scan_next(struct parser* p)
{
    ...
    while (p->head != p->end)
    {
        if (p->data[p->head] == '/' && p->data[p->head + 1] == '*')
        {
            ...
            if (p->head == p->end)
                return print_parser_error(p, "Missing closing comment\n");
...
static void ll_prepend(struct ll** head, struct ll* node)
{
    node->next = *head;
    *head = node;
}
...
static void strlist_add(struct strlist** head, struct str_view string)
{
...
static int parse_data_type_range(struct parser* p, struct data_type* range)
{
    enum token tok;
    int        sign = 1;
    ...
    if (tok != TOK_INTEGER)
        return print_parser_error(p, "Expected integer after '['\n");
    range->minval = range->maxval = p->value.integer * sign;
    ...
    if (tok != '-')
        return print_parser_error(p, "Expected '-' or ']' after integer\n");
    ...
    if (tok != TOK_INTEGER)
        return print_parser_error(p, "Expected integer after '-'\n");
    range->maxval = p->value.integer * sign;
    if (range->maxval < range->minval)
        return print_parser_error(
            p,
            "Invalid range: minval %d > maxval %d\n",
            range->minval,
            range->maxval);
...
static int parse_data_type(struct parser* p, struct data_type* dt)
{
    enum token tok = scan_next(p);
    if (tok != TOK_IDENTIFIER)
        return tok;

    if (cstr_equal("str", p->value.str, p->data))
        dt->type = DT_STR_DYNAMIC;
    else if (cstr_equal("bool", p->value.str, p->data))
        dt->type = DT_BOOL;
    else if (cstr_equal("u", p->value.str, p->data))
        dt->type = DT_UBITFIELD;
    else if (cstr_equal("i", p->value.str, p->data))
        dt->type = DT_IBITFIELD;
    else if (cstr_equal("u8", p->value.str, p->data))
        dt->type = DT_U8;
    else if (cstr_equal("i8", p->value.str, p->data))
        dt->type = DT_I8;
    else if (cstr_equal("u16", p->value.str, p->data))
        dt->type = DT_U16;
    else if (cstr_equal("i16", p->value.str, p->data))
        dt->type = DT_I16;
    else if (cstr_equal("u32", p->value.str, p->data))
        dt->type = DT_U32;
    else if (cstr_equal("i32", p->value.str, p->data))
        dt->type = DT_I32;
    else
        return print_parser_error(
            p,
    ...
    if (tok == '[')
    {
        tok = parse_data_type_range(p, dt);
        if (tok != ']')
            return print_parser_error(
                p, "Expected ']' after data type range\n");
        tok = scan_next(p);
    }

    if (dt->type == DT_STR_DYNAMIC)
    {
        if (dt->minval != dt->maxval)
            return print_parser_error(
                p, "String length must be fixed, not a range\n");
        if (dt->minval < 0)
            return print_parser_error(p, "String length must be positive\n");
        if (dt->minval > 0)
            dt->type = DT_STR_FIXED;
    }

    if (dt->type == DT_BOOL)
    {
        if (dt->minval != 0 || dt->maxval != 0)
            return print_parser_error(
                p,
...
static int parse_section(struct parser* p)
{
    ...
    while (1)
    {
        ...
        tok = parse_data_type(p, &dt);
        if (tok != TOK_IDENTIFIER)
            return tok;
        property = p->value.str;
...
static int parse(struct parser* p, struct root* root)
{
    ...
    while (1)
    {
        ...
        switch (tok)
        {
            ...
            case TOK_IDENTIFIER: {
                struct str_view section = p->value.str;
                if (scan_next(p) != '{')
                    return print_parser_error(
                        p, "Expected '{' after section name\n");
                if (parse_section(p) != '}')
                    return print_parser_error(
                        p,
...
static void
gen_ini_parser(struct mstream* ms, const struct root* root, const char* data)
{
}
...
int main(int argc, char** argv)
{
    ...
    gen_ini_parser(&ms, &root, mf.address);
