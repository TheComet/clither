...
struct NAME : public Test
{
    ...
    void TearDown() override { rbobj_deinit(rbobj); }
...
TEST_F(NAME, space_left)
{
    ...
    rbobj->read = 5;
    rbobj->write = 8;
    EXPECT_THAT(rb_space(rbobj), Eq(32 - 4));
    rbobj->read = 8;
    rbobj->write = 5;
    EXPECT_THAT(rb_space(rbobj), Eq(2));
    rbobj->read = 16;
    rbobj->write = 15;
    EXPECT_THAT(rb_space(rbobj), Eq(0));
    ...
    rbobj->read = 32 - 1;
    rbobj->write = 0;
    EXPECT_THAT(rb_space(rbobj), Eq(32 - 2));
}
...
TEST_F(NAME, is_full_and_is_empty_macros)
{
    rbobj->read = 2;
    rbobj->write = 2;
    EXPECT_THAT(rb_is_full(rbobj), IsFalse());
    EXPECT_THAT(rb_is_empty(rbobj), IsTrue());
    EXPECT_THAT(rb_is_full(rbobj), IsFalse());
    EXPECT_THAT(rb_is_empty(rbobj), IsTrue());
    ...
    rbobj->write = 3;
    EXPECT_THAT(rb_is_full(rbobj), IsFalse());
    EXPECT_THAT(rb_is_empty(rbobj), IsFalse());
    EXPECT_THAT(rb_is_full(rbobj), IsFalse());
    EXPECT_THAT(rb_is_empty(rbobj), IsFalse());

    rbobj->read = 3;
    rbobj->write = 2;
    EXPECT_THAT(rb_is_full(rbobj), IsTrue());
    EXPECT_THAT(rb_is_empty(rbobj), IsFalse());
    EXPECT_THAT(rb_is_full(rbobj), IsTrue());
    EXPECT_THAT(rb_is_empty(rbobj), IsFalse());
...
TEST_F(NAME, put)
{
    EXPECT_THAT(rb_count(rbobj), Eq(0));
    ...
    EXPECT_THAT(rb_count(rbobj), Eq(3));
...
TEST_F(NAME, take)
{
    ...
    ASSERT_THAT(rbobj_take(rbobj), Eq(obj{0xa, 0xa, 0xa, 0xa}));
    ASSERT_THAT(rbobj_take(rbobj), Eq(obj{0xb, 0xb, 0xb, 0xb}));
    ASSERT_THAT(rbobj_take(rbobj), Eq(obj{0xc, 0xc, 0xc, 0xc}));
...
TEST_F(NAME, put_and_take_wrap)
{
    ...
    for (int i = 0; i != rbobj->capacity * 64; ++i)
    {
        ...
        // read 3 bytes
        EXPECT_THAT(rbobj_take(rbobj), Eq(obj{0xa, 0xa, 0xa, 0xa}));
        EXPECT_THAT(rbobj_take(rbobj), Eq(obj{0xb, 0xb, 0xb, 0xb}));
        EXPECT_THAT(rbobj_take(rbobj), Eq(obj{0xc, 0xc, 0xc, 0xc}));
...
TEST_F(NAME, if_it_dont_fit_dont_shit_single)
{
    ...
    ASSERT_THAT(rb_space(rbobj), Eq(0));
    ASSERT_THAT(rb_is_full(rbobj), IsTrue());
    ...
    while (num_left--)
        ASSERT_THAT(
            rbobj_take(rbobj), Eq(obj{num_left, num_left, num_left, num_left}));

    ASSERT_THAT(rbobj_take(rbobj), Eq(obj{0xa, 0xa, 0xa, 0xa}));

    ASSERT_THAT(rb_space(rbobj), Eq(rbobj->capacity - 1));
    ASSERT_THAT(rb_is_empty(rbobj), IsTrue());
...
TEST_F(NAME, peek)
{
    ...
    // Read everything back
    num_left = 32 - 1;
    for (int i = 0; i != rb_count(rbobj); ++i)
    {
        num_left--;
        ASSERT_THAT(
            rb_peek(rbobj, i),
