...
{
public:
    void              SetUp() override { qwpos_vec_init(&points); }
    void              TearDown() override { qwpos_vec_deinit(points); }
    struct qwpos_vec* points;
};
...
TEST_F(NAME, misfit)
{
     ...
     * px = [36550, 36503, 36453, 36401, 36346, 36291, 36234, 36174, 36112,
     * 36047, 35982, 35915, 35845, 35773, 35701, 35627, 35552, 35475, 35396,
     * 35317, 35236, 35153, 35069, 34983, 34895, 34807, 34717, 34626, 34533,
     * 34438, 34342, 34247, 34151, 34055, 33959, 33863, 33766, 33669, 33573,
     * 33477, 33381, 33284, 33188, 33091, 32995, 32898, 32800, 32703, 32605]; py
     * = [25912, 26013, 26113, 26211, 26308, 26405, 26501, 26596, 26689, 26781,
     * 26873, 26964, 27054, 27142, 27230, 27316, 27403, 27488, 27571, 27654,
     * 27735, 27814, 27894, 27972, 28048, 28124, 28198, 28269, 28338, 28405,
     * 28469, 28533, 28594, 28654, 28712, 28766, 28818, 28867, 28915, 28963,
     * 29008, 29051, 29093, 29133, 29173, 29210, 29245, 29280, 29312];
     */
    struct bezier_knot head = {{0, 0}, 0, 0, 0};
    ...
    for (int i = 0; i != array_len(points3); ++i)
        qwpos_vec_push(&points, points3[i]);
