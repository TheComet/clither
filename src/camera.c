...
{
    qw leadx = make_qw(cos(qa_to_float(snake->head_angle)) / 32);
    qw leady = make_qw(sin(qa_to_float(snake->head_angle)) / 32);
    qw targetx = qw_add(snake->head_pos.x, leadx);
    qw targety = qw_add(snake->head_pos.y, leady);
