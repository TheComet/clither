...
snake_param_update(struct snake_param* param, struct snake_upgrades upgrades, uint32_t food_eaten)
{
...

    {
        ...
        qa degrade_fac = make_qa2(degrade_scale, (int)food_eaten > degrade_scale ? food_eaten : degrade_scale);
    ...
    param->cached_stats.scale = qw_add(make_qw(1), make_qw2(food_eaten, 1024 * 4));
