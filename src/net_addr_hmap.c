...
static int net_addr_hmap_kvs_alloc(
    struct net_addr_hmap_kvs* kvs,
    struct net_addr_hmap_kvs* old_kvs,
    int16_t                   capacity)
{
    (void)old_kvs;
...
static void net_addr_hmap_kvs_set_key(
    struct net_addr_hmap_kvs* kvs, int16_t slot, const struct net_addr* key)
{
    kvs->keys[slot].len = key->len;
    memcpy(kvs->keys[slot].sockaddr_storage, key->sockaddr_storage, key->len);
}

static int net_addr_hmap_kvs_keys_equal(
...
static void net_addr_hmap_kvs_set_value(
    struct net_addr_hmap_kvs* kvs, int16_t slot, const int* value)
{
...
HMAP_DEFINE_FULL(
    ...
    int,
    16,
    net_addr_hmap_kvs_hash,
    net_addr_hmap_kvs_alloc,
    net_addr_hmap_kvs_free_old,
    net_addr_hmap_kvs_free,
    net_addr_hmap_kvs_get_key,
    net_addr_hmap_kvs_set_key,
    net_addr_hmap_kvs_keys_equal,
    net_addr_hmap_kvs_get_value,
    net_addr_hmap_kvs_set_value,
    128,
    70)
